    ...
    <>
      ...
      {pair && data && (
        <>
          ...
          <section className="section-stack" aria-labelledby="services-title"><div className="section-heading"><div><h2 id="services-title">DEX Screener services</h2><p>Prices can change. Unverified prices are deliberately replaced with “Check Current Price”.</p></div><a className="btn" href="https://marketplace.dexscreener.com/" target="_blank" rel="noreferrer">Open Marketplace <ArrowUpRight size={16} /></a></div><div className="terminal-panel panel-body service-list">{data.services.map((service) => <div className="service-row" key={service.name}><div><strong>{service.name}</strong><div className="service-price">{service.priceLabel}</div></div><p>{service.description}<br />Last price check: {service.lastChecked}</p><a className="btn" href={service.officialUrl} target="_blank" rel="noreferrer">Official Link <ArrowUpRight size={15} /></a></div>)}</div></section>

          <section className="section-stack" aria-labelledby="update-title"><div className="section-heading"><div><h2 id="update-title">Update token on DEX Screener</h2><p>Current token page information detected from the selected pair.</p></div></div><div className="terminal-panel"><div className="panel-body status-list"><StatusItem label="Logo" present={Boolean(pair.info?.imageUrl)} /><StatusItem label="Website" present={Boolean(pair.info?.websites?.length)} /><StatusItem label="X / Twitter" present={Boolean(pair.info?.socials?.some((social) => `${social.type ?? social.platform}`.toLowerCase().includes("twitter")))} /><StatusItem label="Telegram" present={Boolean(pair.info?.socials?.some((social) => `${social.type ?? social.platform}`.toLowerCase().includes("telegram")))} /><StatusItem label="Description" present={false} /><StatusItem label="Paid profile order" present={data.orders.some((order) => order.type === "tokenProfile" && order.status === "approved")} /></div><div className="panel-body token-actions"><a className="btn btn-primary" href="https://marketplace.dexscreener.com/" target="_blank" rel="noreferrer">Update Token Info <ArrowUpRight size={16} /></a><a className="btn" href="https://marketplace.dexscreener.com/" target="_blank" rel="noreferrer">View Current Pricing</a></div></div></section>
...
function StatusItem({ label, present }: { label: string; present: boolean }) { return <div className="status-item" data-present={present}>{present ? <Check size={16} /> : <Info size={16} />} {label}: {present ? "Available" : "Not detected"}</div> }
